...
func (c *connection) read() {
	...
	for {
		...
		if c.availableRead > 0 {
			...
			if err == io.EOF {
				logger.Info("connection read() - err io.EOF, closing the connection")
				defer c.conn.Close()
				defer close(c.chRead)
				return;
			}
			if err != nil {
				logger.Errorf("connection read() - err %w, closing the connection", err)
				defer c.conn.Close()
				defer close(c.chRead)
				return;
			}
		...
		if errProtocol == nil || errProtocol.Status() == needMoreConnRead {
			if processed < count {
				...
				logger.Debugf("connection read() - consolidating read buffer after processor handling - processed bytes %d, available bytes %d", processed, count)
				...
				c.availableRead = 0
				logger.Debugf("connection read() - processor processed all available bytes %d", processed)
			} else {
				logger.Errorf("connection read() - processor return invalid processed value %d, count %d, closing connection", processed, count)
				defer c.conn.Close()
				defer close(c.chRead)
				return;
			}
		...
		if errProtocol != nil {
			if errProtocol.Status() == needMoreConnRead {
				...
				logger.Errorf("connection read() - errProtocol %s, closing the connection", errProtocol)
				defer c.conn.Close()
				defer close(c.chRead)
				return;
			}
